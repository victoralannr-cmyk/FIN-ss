...
export const CategoryExpensesChart: React.FC<CategoryExpensesChartProps> = ({ transactions }) => {
  ...
  const categoryMap = transactions
    ...
    // Fixed: Removed explicit generic <Record<string, number>> to prevent misinterpretation as JSX in .tsx files
  ...
  return (
    <div className="flex flex-col h-full">
      <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-white mb-8">GASTOS POR CATEGORIA</h3>
      
      {total === 0 ? (
        <div className="flex-1 flex items-center justify-center border border-dashed border-neutral-800 rounded-2xl py-12">
          <p className="text-[10px] text-neutral-600 uppercase font-black tracking-widest">Sem dados de gastos</p>
        </div>
      ) : (
            ...
            {data.slice(0, 5).map((item, i) => (
              <div key={i} className="flex items-center justify-between group">
                ...
                <span className="text-[10px] font-black text-white ml-4">
                  R$ {(item.amount as number).toLocaleString('pt-BR')}
                </span>
              </div>
            ))}
            {data.length > 5 && (
